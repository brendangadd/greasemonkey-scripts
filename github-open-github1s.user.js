...
const BTN_CONFIG = [
  ...
  },
  {
    ...
    size: 12,
    className: "btn btn-sm d-inline-block float-none",
    locator: () => document.querySelector(".gh-header-actions"),
  },
  {
    test() {
      return pathMatch("/*/*/blob/**") && document.querySelector("#blob-path");
    },
    size: 16,
    className: "btn ml-2 d-none d-md-block",
    locator: () => document.querySelector("#blob-path").parentElement,
  },
];

function main() {
  // GitHub doesn't load a new page on navigation, so standard events
  // are only fired on first load. Thus, we have to watch the page
  // content for mutations.
  new MutationObserver(maybeRefresh).observe(document.body, {
    childList: true,
    subtree: true,
  });
  refresh();
...
function refresh() {
  const { container, config } = findContainerAndConfig();
  ...
  url.host = "github1s.com";

  /* global */ button = document.createElement("a");
  button.className = config.className;
  button.href = url.href;
  button.innerHTML = `${vsCodeIcon(config.size)} Open`;

  container.appendChild(button);
}

function findContainerAndConfig() {
  for (let config of BTN_CONFIG) {
    if (!config.test()) {
      continue;
    }
    const container = config.locator();
...
function pathMatch(path) {
  const CONVERSIONS = ["[^/]+", ".*"];
  const pathPattern = path.replace(/\*+/g, (match) => {
    const pattern = CONVERSIONS[match.length - 1];
    if (!pattern) throw new Error("Invalid path matcher");
    return pattern;
  });
  return new RegExp(`^${pathPattern}$`).test(window.location.pathname);
}

function vsCodeIcon(size) {
  return `
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" width="${size}" height="${size}" fill="none" class="octicon">
